...
async function seed() {
  try {
    ...
    if (existingCount > 0 && !forceClear) {
      console.log(`Database already has ${existingCount} posts. Skipping seed to preserve existing data. (Use --force to reset).`);
      process.exit(0);
    }

    if (forceClear) {
      console.log("Clearing existing posts (--force specified)...");
    ...
    console.log("Successfully seeded dummy blog posts!");
    process.exit(0);
  } catch (error) {
